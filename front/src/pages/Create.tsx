...
const Create: React.FC = () => {
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    try {
      await httpClient.post('/budget', {
        name: budgetObject.name,
        description: budgetObject.description,
        provided_budget: Number(budgetObject.provided_budget),
        budget_item: budgetObject.budget_item,
      });
  ...
  const conditionalForm = () => {
    return (
      <div className="w-full max-w-md p-8 bg-white rounded-lg shadow-md hover:shadow-xl">
        ...
        <h2 className="text-2xl font-semibold text-center text-darkRed mb-6">
          Faça já um orçamento para a sua pizzaria!
        </h2>
